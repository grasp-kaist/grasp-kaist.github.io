...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  return {
    ...
    databasePath: resolve(env.DATABASE_PATH?.trim() || './data/grasp-profile-bot.sqlite'),
    membersPageUrl: env.MEMBERS_PAGE_URL?.trim() || 'https://grasp-kaist.github.io/members/',
    discord: {
      applicationId: requireValue(env, 'DISCORD_APPLICATION_ID'),
      publicKey: requireValue(env, 'DISCORD_PUBLIC_KEY'),
      guildId: requireValue(env, 'DISCORD_GUILD_ID'),
